...
async def vad_asr_test_websocket(
    ...
    try:
        # 注册连接
        session_id = f"vad_asr_{actual_app_id}"
        await connection_manager.connect(
            websocket=websocket,
            session_id=session_id,
            user_id="vad_asr_test",
            metadata={"app_id": actual_app_id}
        )
        ...
        logger.info(f"VAD+ASR test WebSocket connected with app_id: {actual_app_id}")
        ...
        while True:
            try:
                ...
                if message_type == "ping":
                    ...
                    try:
                        ...
                        if not (0 <= level <= 3):
                            await connection_manager.send_message(
                                session_id,
                                {"type": "error", "content": f"VAD级别必须在0-3之间，收到: {level}"}
                            )
                            continue
                        ...
                        if vad_service:
                            ...
                            await connection_manager.send_message(
                                session_id,
                                {"type": "warning", "content": "VAD服务未初始化，请先启动测试"}
                            )
                    except ValueError:
                        await connection_manager.send_message(
                            session_id,
                            {"type": "error", "content": f"无效的VAD级别: {level}"}
                        )
                    ...
                    if not audio_data:
                        await connection_manager.send_message(
                            session_id,
                            {"type": "error", "content": "音频数据为空"}
                        )
                        continue

                    await _process_vad_asr_audio(
                        session_id, audio_data, audio_format,
                        audio_buffer, actual_app_id, vad_service
                    )
                    ...
                    await connection_manager.send_message(
                        session_id,
                        {"type": "error", "content": f"未知消息类型: {message_type}"}
                    )

            except WebSocketDisconnect:
                logger.info(f"VAD+ASR test WebSocket disconnected")
                break

            except json.JSONDecodeError as e:
                logger.error(f"Invalid JSON in VAD+ASR test: {e}")
                await connection_manager.send_message(
                    session_id,
                    {"type": "error", "content": "JSON格式错误"}
                )
                ...
                logger.error(f"Error in VAD+ASR test: {e}")
                await connection_manager.send_message(
                    session_id,
                    {"type": "error", "content": f"消息处理错误: {str(e)}"}
                )

    except Exception as e:
        logger.error(f"VAD+ASR test WebSocket connection error: {e}")

    finally:
        await connection_manager.disconnect(session_id)


async def _process_vad_asr_audio(
    session_id: str,
    audio_data: str,
    audio_format: str,
    audio_buffer: list,
    ...
    try:
        ...
        if audio_format == 'pcm':
            ...
            if len(combined_audio) >= target_buffer_size:
                logger.info(f"Processing large audio chunk: {len(combined_audio)} bytes")
        ...
        logger.error(f"Error in VAD+ASR audio processing: {e}")
        await connection_manager.send_message(
            session_id,
            {"type": "error", "content": f"音频处理失败: {str(e)}"}
        )
